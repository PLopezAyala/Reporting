...
format: 
  html: 
    ...
    toc-depth: 5 #Depth of headers. If not specified it defaults to 3 (meaning that all level 1, 2, and 3 headers will be included in the table of contents)
    toc-location: left #choose position of table of content. default is right
    code-fold: true  #enables you to include R code but have it hidden by default 
    code-tools: true #tools for readers to interact with the source code
    number-sections: false #include numbers in your TOC (default is true)
    embed-resources: true # Produce a standalone HTML file with no external dependencies
    smooth-scroll: true #Enable smooth scrolling within the page. By default, smooth scroll is not enabled
    link-external-newwindow: true # true to open external links in a new browser window or tab
    link-external-icon: true #true to show an icon next to the link to indicate that it's external
    theme: default #bootstrap 5 default. All themes can be found here https://quarto.org/docs/output-formats/html-themes.html
editor: source
execute: 
  echo: true #with execute we set global options for code chunks so that all latter chunks will use these options. 
  warning: false
  message: false
...
This `.qmd` file was written with the aim to aid you when writing your first reports in Quarto. It contains the most important features for creating a good reproducible report. 

The structure followed is not necessarily the one you should follow when performing your data analysis or reporting your results. The purpose of this template is simply to present you with a good structure for the data report and the code behind it.

The structure used in this Quarto report is the following:
...
Through this report I have used different chunk options. Below a short summary of the ones I have used. For a detail explanation of these and many other chunk options, please check [this](https://quarto.org/docs/reference/formats/html.html). 
...
* `eval: false` do not evaluate the code (default is `true`) but the code appears if `echo: true` was chosen
* `include: false` to hide both the code and the result from the rendered document but the code is still evaluated
* `label:` For figures produced by executable code cells, include a `label` with a `fig-` prefix to make them cross-referenceable (and for ordering the Figures, e.g. Figure 1, Figure 2). For a table, use the `tbl-` prefix.
* `fig-align:` to align the figure. Four options exist `default`, `left`, `right` or `center`
* `fig-cap:` write a figure caption
* `fig-height:` height of the plot. Default is 5
* `fig-width:` width of the plot. Default is 7
...
* `fig-dpi:` dots per inch value (for image resolution). Default is 72
* `tbl-cap:` write a table caption

::: {.callout-important}

One important difference between R Markdown documents and Quarto documents is that in Quarto, chunk options are typically included using the special comment `#|` at the top of code chunks rather than within the line that begins the chunk. 

```{{r}}
#| echo: true
#| label: fig-myfig
#| fig-cap: title of figure
#| fig-align: center
...
Note that this `.qmd` file was written with the intention to knit to HTML. I believe that HTML files have more advantages than pdf or word files and are easier to present to your PI or colleagues.
...
#| fig-align: center
#| out-width: '70%'
#| fig-cap: GUSTO-I trial published in the NEJM
...
#| label: fig-flowchart
#| fig-align: center
#| fig-cap: Patient flowchart using the {consort} package
#| fig-dpi: 100
...
When downloading the `summarytools` package from CRAN, you will also need to install Xquartz to take advantage of all the features in `summarytools`. You can download it from [here](https://www.xquartz.org). 
...
```{r }
#| label: tbl-summary
#| tbl-cap: Summary statistics using the {summarytools} package
#| results: asis
...
d <- Hmisc::describe(df)
Hmisc::html(d, scroll = TRUE) #scroll = TRUE is for scrolling the output table
...
#| label: fig-hmisc-cat
#| fig-align: center
#| fig-cap: Descriptive statistics of categorical variables
...
```{r }
#| label: fig-hmisc-cont
#| fig-align: center
#| fig-cap: Descriptive statistics of continuous variables
...
#| label: fig-hmisc-na
#| fig-align: center
#| fig-cap: Fractions of NAs per Variable
#| fig-height: 7
...
Yes, you can insert equations in Quarto with LaTeX notation. Take a look at different [LaTeX symbols](https://oeis.org/wiki/List_of_LaTeX_mathematical_symbols).
...
#| label: fig-heatmap
#| fig-align: center
#| fig-cap: Heatmap showing predicted probabilities at different ages and systolic blood pressure for the 4 Killip classes
...
Hope this template will aid you when creating a `.html` report file with Quarto. Note that Quarto allows [visual editing mode](https://quarto.org/docs/visual-editor/) (just like R Markdown), making all aspects of creating and rendering a report very easy. 
